...
# Full HTML copied from DemoWebHandler
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .agents-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr));
                       gap: 20px; margin-bottom: 30px; }
        .agent-card { background: white; border: 2px solid #e1e5e9; border-radius: 15px;
                      padding: 20px; text-align: center; transition: transform .3s ease, box-shadow .3s ease; }
        .agent-card:hover { transform: translateY(-5px); box-shadow: 0 10px 25px rgba(0,0,0,0.1); }
        ...
        .search-section { background: #f8f9fa; border-radius: 15px; padding: 25px; margin-bottom: 30px; }
        .search-form { display: flex; gap: 15px; margin-bottom: 20px; }
        #queryInput { flex: 1; padding: 15px 20px; border: 2px solid #e1e5e9; border-radius: 10px;
                      font-size: 16px; outline: none; transition: border-color .3s ease; }
        #queryInput:focus { border-color: #667eea; }
        #searchBtn { padding: 15px 30px; background: linear-gradient(135deg,#667eea 0%,#764ba2 100%);
        ...
        .results-section { background: white; border-radius: 15px; padding: 25px; border: 2px solid #e1e5e9; }
        .result-item { border: 1px solid #e9ecef; border-radius: 10px; padding: 20px; margin-bottom: 20px;
                       background: #fdfdfd; }
        .result-header { display: flex; justify-content: space-between; align-items: center;
                         margin-bottom: 15px; padding-bottom: 10px; border-bottom: 1px solid #e9ecef; }
        .result-query { font-size: 1.2em; font-weight: bold; color: #333; }
        .result-time { color: #666; font-size: 0.9em; }
        .agent-flow { display: flex; justify-content: space-between; margin-bottom: 15px; flex-wrap: wrap; gap: 10px; }
        .flow-step { background: #667eea; color: white; padding: 8px 15px; border-radius: 20px; font-size: 0.9em; font-weight: bold; }
        .result-summary { background: white; border: 1px solid #e9ecef; border-radius: 8px; padding: 15px;
        ...
        .example-queries { display: flex; flex-wrap: wrap; gap: 10px; margin-top: 10px; }
        .example-query { background: #e9ecef; padding: 5px 12px; border-radius: 15px; cursor: pointer; transition: background .2s ease; }
...
<body>
    <div class="container">
        ...
        <div class="agents-grid">
            <div class="agent-card"><div class="agent-icon">🔍</div><div class="agent-name">Search Agent</div><div class="agent-desc">Finds and retrieves relevant information from multiple sources</div></div>
            <div class="agent-card"><div class="agent-icon">📝</div><div class="agent-name">Summarizer Agent</div><div class="agent-desc">Creates comprehensive summaries and extracts key insights</div></div>
            <div class="agent-card"><div class="agent-icon">✅</div><div class="agent-name">Validator Agent</div><div class="agent-desc">Validates information quality and provides confidence scores</div></div>
        </div>
        ...
        <div class="results-section">
            ...
            <div id="results"><p style="color: #666; text-align: center; padding: 40px;">Enter a research query above to see the multi-agent system in action!</p></div>
        </div>
    </div>
<script>
let currentResultCount = 0;
let currentQuery = '';
function setQuery(query) { document.getElementById('queryInput').value = query; }
async function startResearch() {
    ...
    try {
        const response = await fetch('/api/research', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({query: query}) });
        if (response.ok) {
        ...
        btn.disabled = false; btn.innerHTML = 'Research';
    }
}
async function pollResults() {
    try {
        ...
        } else { setTimeout(pollResults, 500); }
    } catch (error) { setTimeout(pollResults, 500); }
}
...
document.getElementById('queryInput').addEventListener('keypress', function(e) { if (e.key === 'Enter') { startResearch(); } });
window.addEventListener('load', function() { fetch('/api/results').then(r => r.json()).then(data => { if (data.results && data.results.length > 0) { displayResults(data.results); } }).catch(() => {}); });
</script>
</body>
</html>"""
